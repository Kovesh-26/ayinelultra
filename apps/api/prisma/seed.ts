...
async function main() {
  ...
  await prisma.membershipTier.create({
    data: {
      ...
      name: 'Member',
      priceCents: 499,
      perks: ['Ad‑free', 'Crew badge'],
    },
  });
  const prod = await prisma.product.create({
    data: { studioId: studio.id, title: 'Creator Tee', priceCents: 2500 },
  });
  ...
  await prisma.orderItem.create({
    data: { orderId: order.id, productId: prod.id, qty: 1, unitCents: 2500 },
  });

  // flags
  await prisma.featureFlag.create({
    data: { key: 'clips-editor', enabled: false, rollout: 0 },
  });
}
