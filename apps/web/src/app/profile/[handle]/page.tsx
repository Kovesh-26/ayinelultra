    ...
    <main className="mx-auto max-w-6xl p-4 space-y-4">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border p-4 md:col-span-2 space-y-3">
          ...
          <div className="rounded-xl border p-3 text-sm">Posted a new video…</div>
          <div className="rounded-xl border p-3 text-sm">Boosted "City Nights"…</div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border p-4">
        ...
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="rounded-xl border p-3 text-center text-sm">friend_{i+1}</div>
          ))}
        </div>
      </section>
    </main>
