...
export default function WatchPageClient({ id }: { id: string }) {
  ...
  const [chatMessages, setChatMessages] = useState([
    { id: 1, user: 'FanUser123', message: 'This is absolutely incredible! 🔥', time: '2:34' },
    { id: 2, user: 'ContentLover', message: 'I\'ve been waiting for this content for so long! 🙏', time: '2:35' },
    { id: 3, user: 'TrendWatcher', message: 'The quality is amazing! What camera do you use? 📹', time: '2:36' },
    { id: 4, user: 'CreatorFan', message: 'This is next level content! 🚀', time: '2:37' },
    { id: 5, user: 'VideoEnthusiast', message: 'Can\'t wait for the next upload! 💯', time: '2:38' }
  ]);
  ...
  const video = {
    ...
    description: "Join me on this incredible journey as we explore the latest trends in content creation. Don't forget to like, comment, and subscribe for more amazing content! #AYINEL #Creator #Trending",
    views: 124756,
    likes: 8923,
    dislikes: 156,
  ...
  const recommendedVideos = [
    ...
    },
    {
      ...
      views: "156K",
      duration: "15:22",
      thumbnail: "https://images.unsplash.com/photo-1551434678-e076c223a692?w=320&h=180&fit=crop"
    },
    {
      id: "4",
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <div className="relative">
        ...
        <div className="relative bg-gray-900 aspect-video group">
          ...
          <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-8 transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="absolute top-6 left-6 flex items-center space-x-3 bg-red-600 px-4 py-2 rounded-full shadow-lg">
            ...
            <span className="text-sm">{formatNumber(liveViewers)} watching</span>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-8 py-8">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  {video.creator.verified && <span className="text-blue-400 text-2xl">✓</span>}
                </div>
                <p className="text-gray-400 text-lg">{video.creator.handle}</p>
                <p className="text-sm text-gray-500">{formatNumber(video.creator.subscribers)} subscribers</p>
              </div>
            </div>
            <button 
          ...
          <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-3xl p-6 border border-purple-500/30 backdrop-blur-sm mb-8">
            <p className="text-gray-300 text-lg mb-4 leading-relaxed">{video.description}</p>
            <div className="flex flex-wrap gap-3">
              {video.tags.map((tag, index) => (
                <span key={index} className="text-purple-400 hover:text-purple-300 cursor-pointer text-lg">
                  {tag}
                </span>
              ))}
            </div>
                ...
                {comments.map((comment) => (
                  <div key={comment.id} className="flex space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-300 text-lg mb-3">{comment.content}</p>
                      <div className="flex items-center space-x-6 text-sm text-gray-400">
                        <button className="hover:text-white transition">👍 {comment.likes}</button>
      ...
      <div className="fixed top-0 right-0 w-80 h-full bg-black/95 backdrop-blur-md border-l border-white/10 overflow-y-auto z-40" style={{ right: showLiveChat ? '320px' : '0' }}>
        <div className="p-6">
