...
import { HeartIcon, MessageCircleIcon, UsersIcon, PlusIcon } from 'lucide-react';
    ...
    <div className="bg-gray-800 border-b border-gray-700">
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-start space-x-4">
            <div className="w-24 h-24 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full flex items-center justify-center text-white text-2xl font-bold -mt-12 border-4 border-gray-800">
              {studio.avatar ? (
                <img src={studio.avatar} alt={studio.name} className="w-full h-full rounded-full object-cover" />
              ) : (
            ...
            <div className="pt-2">
              ...
              {studio.description && (
                <p className="text-gray-300 mt-2 max-w-2xl">{studio.description}</p>
              )}
              <div className="flex items-center space-x-4 mt-3 text-sm text-gray-400">
                <span>{studio.followerCount.toLocaleString()} Crew members</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex space-x-8 mt-6 border-b border-gray-700">
          {['About', 'Videos', 'Music', 'Broadcast', 'Community', 'Store'].map((tab) => (
            <button
              key={tab}
              className={cn(
