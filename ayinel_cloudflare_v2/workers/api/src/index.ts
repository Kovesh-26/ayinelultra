...
app.use('/*', cors({ origin: '*', allowHeaders: ['*'], allowMethods: ['GET','POST','PUT','PATCH','DELETE','OPTIONS'] }))

app.get('/health', (c) => c.json({ ok: true, service: 'ayinel-api' }))

app.get('/profiles/:handle', async (c) => {
  const handle = c.req.param('handle')
  const row = await c.env.DB.prepare('SELECT id, handle, displayName, bio, avatarUrl, bannerUrl, theme, musicUrl FROM users WHERE handle = ?').bind(handle).first()
...
app.get('/studios/:handle', async (c) => {
  const handle = c.req.param('handle')
  const row = await c.env.DB.prepare('SELECT id, handle, name, about, bannerUrl, theme, musicUrl, links, tipsLinks FROM studios WHERE handle = ?').bind(handle).first()
  if (!row) return c.json({ error: 'not_found' }, 404)
  return c.json(row)
})
...
app.post('/media/upload-url', async (c) => {
  ...
  if (c.env.STREAM_ACCOUNT_ID && c.env.STREAM_API_TOKEN) {
    ...
    if (!data?.success) return c.json({ error: 'stream_direct_upload_failed', details: data }, 500)
    return c.json({ uploadURL: data?.result?.uploadURL, uid: data?.result?.uid, kind })
  }
